...
- **Multi-language by contract** — every portable contract is specified at the byte level (envelope, schema hash, topic/QoS mapping), so non-C++ components join through their backend's native bindings plus the spec; dedicated bindings only when a consumer demands one (priority: C++ first, then Python, Go, Rust).
