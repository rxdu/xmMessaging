...
- A4 *(amended at P0a, see D19)*: the universal payload floor and the zero-copy loan requirement are **compile-time** errors at wiring; per-reach requirements beyond them (cross-language layout, network serializer) surface as an explicit wiring **status** — reach is a runtime property of `Domain`, so full per-reach compile enforcement is impossible without putting reach in the type system, which would break this scenario's A1. Named traits (`is_cross_language_payload_v`, …) let an application that knows its reach static_assert anyway.
...
| D19 | P0a freeze | Reach stays **runtime** (one `Domain` type, factory-selected): compile-time payload enforcement = universal floor at endpoint instantiation + trivially-copyable at `Loan<T>`; per-reach requirements are wiring statuses + opt-in named traits. M6-A4 amended accordingly. |
| D20 | P0a freeze | Status refinements: `PublishStatus` stays two-valued `{kOk, kWouldBlock}` (D8 accounting is total) — loan exhaustion is `LoanStatus` on the loan handle; `ReplyStatus::kExpired` makes server-side late-reply discard observable; consumer context adoption uses xmBase's real `ContextGuard` name (D13's `ContextScope` was a placeholder); `WaitForWorkOrShutdown` is a `Server` member taking an explicit max-park bound; `Publish` is deliberately not `[[nodiscard]]` (best-effort discards by design, D8) — the cookbook must call out that reliable publishers should read it. |
