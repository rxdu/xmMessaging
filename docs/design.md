...
Bindings policy: on demand, not speculative (the R3/no-speculation discipline). When the first real consumer wants one — Python is the expected first, for tooling, tuners, and ML-side glue — it is built over a small C ABI on the portable core, and the wire-contract spec is what keeps it honest.
